      ...
      <div className="max-w-screen-xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        {isHome && (
          <nav className="hidden lg:flex items-center gap-0.5">
            {links.map(l => (
              <a key={l.href} href={l.href} className="px-3 py-1.5 text-xs font-semibold text-gray-500 hover:text-[#0a0a0a] hover:bg-gray-50 rounded-md transition-colors">
                {l.label}
              </a>
            ))}
          </nav>
        )}
        <a href="https://www.joynthefun.com" target="_blank" rel="noopener noreferrer" className="text-xs font-bold text-[#FD5C1E] hover:underline">
          joynthefun.com ↗
        </a>
      </div>
    </header>
